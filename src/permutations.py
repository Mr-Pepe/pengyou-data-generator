    ...
    try:
        ...
        for entry in entries:
            ...
            if len(syllables) <= max_len_permutations:
                ...
                if len(hanzi) != len(syllables):
                    tmp_permutations = create_permutations(['' for x in syllables[:max_len_permutations]],
                                                           syllables[:max_len_permutations])[1:]
                else:
                    tmp_permutations = create_permutations(hanzi[:max_len_permutations],
                                                           syllables[:max_len_permutations])
...
def create_permutations(hanzi, syllables):
    if len(syllables) == 1:
        syllable = syllables[0].lower()
        hanzi = hanzi[0].lower()

        if syllable == hanzi:
            output = [syllable]
        else:
            output = [hanzi, syllable]
